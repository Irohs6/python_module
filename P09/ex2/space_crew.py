    ...
    def custom_duration_validator(self):
        ...
        if self.duration_days > 365:
            ...
            if veterant_member >= len(self.crew) / 2:
                return self
            raise ValueError(
                "Missions longer than 1 year require at least "
...
def main():
    ...
    for i, space_mission in enumerate(data):
        try:
            space_mission = SpaceMission(**space_mission)
            ...
            print("=" * 50)
            print(f"[KO] mission {i+1}: {data[i].get('mission_id', '?')}")
