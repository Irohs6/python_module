...
class PixelDataGenerator:
    ...
    def _command_for_quest(
        self, count: int = 8, format_type: str = "list"
    ) -> Any:
    ...
    def _scores_for_analytics(
        self, count: int = 10, format_type: str = "list"
    ) -> Any:
    ...
    def _analytics_for_dashboard(self) -> Dict[str, Any]:
        ...
        players_data = {
            player: {
                ...
                "favorite_mode": random.choice(
                    ["casual", "competitive", "ranked"]
                ),
                "achievements_count": random.randint(
                    1, len(self.achievements)
                ),
            }
            for player in self.players
        }
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        try:
            ...
            print(f"Error: {e}")
            print(
                "Usage: python3 data_generator.py <exercise_num> "
                "[--count N] [--format argv|list]"
            )
    else:
        # Interactive mode
